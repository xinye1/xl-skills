...
| `handover` | Owns everything at the phase boundary: exit-criteria verification, phase memory, the handover prompt and its anti-patterns. Its execution-model references (`execution-model.md`, or `execution-model-goal-directed.md` on an opus or fable chat) are also Step 2's orchestration model. |
