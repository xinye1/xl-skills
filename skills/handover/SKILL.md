...
| Recommending `fable` or `opus` "to be safe" | The premium tiers pay off only where the orchestration or task is genuinely beyond the tier below (pricing scales ~1:3:5:10 haiku→fable). On routine phases they burn 2–10× the spend for the same outcome. Size the model to the phase — in both directions. |
