...
**Primary reviewer:** the **CodeRabbit CLI, run inline from the orchestrator** — a separate inference process *plus* deterministic static analysis, so it is genuinely **independent** of a Claude coding agent that authored the code. It reviews **local** changes directly — no PR or push needed — which is what lets the push-coupled path review pre-push.

**Run it inline, NOT via the `coderabbit:code-reviewer` subagent.** The subagent summarises the CLI's output, and a rate-limit abort (`✗ Review limit reached … wait N minutes`, exit 1, **nothing reviewed**) summarises to the same thing a genuine clean pass does: "APPROVE, no findings". The distinguishing evidence — the error string and the exit code — is exactly what the summarising layer discards, so a merge gate can go green on a review that never ran. Measured 2026-07-03: a subagent returned APPROVE on a commit where the CLI had hit the limit and reviewed nothing. The failure is silent and intermittent, so "it has worked so far" is not evidence against it. A subagent is acceptable **only** when it invokes a wrapper that fails loud on abort and writes the verdict somewhere durable, and reports just that wrapper's exit code — never when it interprets raw CLI output.

Capture the run (`> file`) and require its own trailer — `Review complete` plus the findings count — as the proof it completed. A global counter such as `~/.coderabbit/stats.json` cannot serve as that proof: the rate bucket is shared, so a concurrent session's review can increment it.

**Fallback:** `feature-dev:code-reviewer` if coderabbit is unavailable — but treat this as a **degraded gate, not an equivalent**: it is a *same-model* reviewer (a Claude subagent reviewing Claude-authored code), so it produces correlated findings and cannot catch failure modes that Claude-class models systematically miss. When the fallback is the only reviewer, lean on the repo's deterministic checks (Semgrep / secret-scan / linters) as the independent backstop and weight the human merge decision accordingly.
...
Point the reviewer at the **diff under review**: `git fetch origin <base> && coderabbit review --base origin/<base>` (add `--committed` when the working tree is dirty — the CLI reviews uncommitted tracked edits by default, which are not what the PR contains). Always a remote-tracking ref; a bare branch name resolves to the local branch, which can sit far behind. The verdict is the run's own trailer — `Review complete` + the findings count — not a paraphrase of it. For the **degraded fallback** subagent, dispatch with the diff (`gh pr diff <pr>` once pushed, *not* the working tree) and require `APPROVE` or `REQUEST_CHANGES` + issue list.

**Preserve the review where the PR can show it.** CodeRabbit's Free plan posts only a summary walkthrough on private-repo PRs — no file:line findings — so a CLI review that stays in the terminal leaves no record of what gated the merge. Publish the captured output to the PR (`gh pr comment <pr> --body-file`, or a repo wrapper such as `scripts/cr_review_to_pr.sh`), and re-publish after folding fixes so the PR carries the review of the tree that actually merged.
...
| `requesting-code-review` | Step 4 owns review dispatch; review is mandatory before every merge. Timing follows CI coupling (Step 4): pre-push when CI runs on push, post-push (free) when CI is gated pre-merge. One primary reviewer per change set — the CodeRabbit CLI run inline (not the `coderabbit:code-reviewer` subagent, which can summarise a rate-limit abort into a false APPROVE); a same-model Claude reviewer is only the degraded fallback. |
