...
| [boil-the-ocean](skills/boil-the-ocean/SKILL.md) | Implementation completeness standard (after Gary Tan): push past "good enough" toward "holy shit, that's done." Five-question done-check, anti-patterns to kill (TODO-the-test, workaround-instead-of-fix, type-check-only), and an explicit reconciliation with the system prompt's "no scope creep" rule — completeness *within* the asked-for scope, not unbounded polish. |
